...
bool Search(const Map<string, Set<string>>& possibleLinks,const Vector<string>& peaple,int index,Set<string>& used,Set<Pair>& matching){
    string A;
    if(used.size()==peaple.size())
        return true;
    for(string p:peaple){
        if(!used.contains(p)){
            A=p;
    ...
    for(string B:possibleLinks[A]){
        if(!used.contains(B)){
            used.add(A);
            used.add(B);
            matching.add(Pair(A,B));
        if(Search(possibleLinks,peaple,index+1,used,matching)){
            return true;}
        matching.remove(Pair(A,B));
        used.remove(A);
        used.remove(B);
        }
    }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    Vector<string> peaple;
    for (string p : possibleLinks) {
        peaple.add(p);
    }
    Set<string> used;
    return Search(possibleLinks, peaple, 0, used, matching);
    (void) possibleLinks;
    (void) matching;
    return false;
}
...
/**
 * 递归回溯函数
 * remainingPeople: 还没决定命运的人
 * graph: 权重图
 * currentScore: 当前路径的权重和
 * currentMatching: 当前路径已选择的配对
 * maxScore: 全局发现的最大权重（引用）
 * bestMatching: 全局发现的最佳配对集合（引用）
 */
void solveMatching(Set<string>& remainingPeople,
                   const Map<string, Map<string, int>>& graph,
                   int currentScore,
                   Set<Pair>& currentMatching,
                   int& maxScore,
                   Set<Pair>& bestMatching) {

    // Base Case: 如果没有人剩下，说明这是一个完整的方案
    ...
    // 确定性选择：取集合中的第一个人作为当前考虑对象 P
    // 这避免了重复生成相同的匹配（例如 {A,B} 和 {B,A}）
    string p = remainingPeople.first();
    remainingPeople.remove(p);

    // ------------------------------------------------
    // 选项 1: P 不与任何人匹配 (P stays single)
    // ------------------------------------------------
    solveMatching(remainingPeople, graph, currentScore, currentMatching, maxScore, bestMatching);

    // ------------------------------------------------
    // 选项 2: P 与某个邻居匹配 (Match P with N)
    // ------------------------------------------------
    // 遍历 P 的所有潜在连接
    // 注意：我们要检查 graph 是否包含 p (防止 p 没有任何连接导致崩溃，虽然通常 key 存在)
    if (graph.containsKey(p)) {
        for (string partner : graph[p]) {
            // partner 必须还在 remainingPeople 中（即 partner 还没被占用）
            if (remainingPeople.contains(partner)) {
                int weight = graph[p][partner];

                // 剪枝：只考虑正权重的边 (负权重或0权重无需匹配)
                if (weight > 0) {
                    // Choose (做出选择)
                    remainingPeople.remove(partner);
                    Pair match(p, partner);
                    currentMatching.add(match);

                    // Explore (递归)
                    solveMatching(remainingPeople, graph, currentScore + weight, currentMatching, maxScore, bestMatching);

                    // Un-choose (回溯/撤销)
                    currentMatching.remove(match);
                    remainingPeople.add(partner);
                }
            }
        }
    }

    // 恢复现场：将 p 放回集合，供上一层递归使用
    remainingPeople.add(p);
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    // 2. 开启递归搜索
    solveMatching(people, possibleLinks, 0, currentMatching, maxScore, bestMatching);
