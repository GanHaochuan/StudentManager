 ...
 * 递归辅助函数
 * roadNetwork: 道路网（不变）
 * numCities: 剩余可用的物资点数量
 * supplyLocations: 当前选中的物资点（结果集）
 * coveredCities: 当前已经被覆盖的城市集合
 */
bool Search(const Map<string, Set<string>>& roadNetwork,int numCities,Set<string>& supplyLocations,Set<string>& coveredCities){
    if(coveredCities.size()==roadNetwork.size())
        return true;
    if(numCities==0)
        return false;
    string targetcity = "";
    for(std::string city:roadNetwork){
        if(!coveredCities.contains(city)){
            targetcity=city;
            break;
        }
    }
    Set<string> candicates = roadNetwork[targetcity];
    candicates.add(targetcity);
    for(string candicate : candicates){
        supplyLocations.add(candicate);
        Set<string> newlyCovered;
        if(!coveredCities.contains(candicate)){
            newlyCovered.add(candicate);
        }
        for(string neighbor : roadNetwork[candicate]){
            if(!coveredCities.contains(neighbor))
                newlyCovered.add(neighbor);
        }
        coveredCities+=newlyCovered;
        if(Search(roadNetwork,numCities-1,supplyLocations,coveredCities)){
            return true;
        }
        supplyLocations.remove(candicate);
        coveredCities-=newlyCovered;
    }
    return false;
}
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    return Search(roadNetwork,numCities,supplyLocations,coveredCities);
    (void) roadNetwork;
    (void) numCities;
