...
Vector<DataPoint> help(Vector<DataPoint>& left,Vector<DataPoint>& right){
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    Vector<Vector<DataPoint>> left;
    Vector<Vector<DataPoint>> right;
    for (int i = 0; i < mid; ++i) {
    ...
    return help(leftcombine,rightcombine);
    (void) sequences;
    return {};
}
