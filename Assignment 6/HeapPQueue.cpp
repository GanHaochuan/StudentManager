...
void HeapPQueue::bubbleUp(int index){
    while(index>1){
        int parent = index/2;
        if(elems[index].weight<elems[parent].weight){
            swap(elems[index],elems[parent]);
            index=parent;
        }else {break;}
    }
}
...
void HeapPQueue::bubbleDown(int index){
    while (true) {
        ...
        int right=left+1;
        int smallest=index;
        if(left<=logicalSize  &&  elems[left].weight<elems[smallest].weight)
            smallest=left;
        if(right<=logicalSize  &&  elems[right].weight<elems[smallest].weight)
            smallest=right;
        if(smallest!=index){
            swap(elems[index],elems[smallest]);
            index=smallest;
        }else{break;}
    }
}
void HeapPQueue::enqueue(const DataPoint& data) {
    ...
    bubbleUp(logicalSize);
}

DataPoint HeapPQueue::peek() const {
    if (isEmpty()) error("Heap is empty.");
    return elems[1];
}
