...
double priorityOf(int population,int seats){
    return population/sqrt(seats*(seats+1));
}
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    int k = populations.size();
    if(numSeats<k)
        error("Seat count must be at least the number of states.");
    Map<string, int> seats; // 记录最终每州席位
    HeapPQueue pq;          // 用来找最大优先级（使用负 priority）
    /* 1. 初始每州分 1 席 */
    for (string state : populations) {
        seats[state] = 1;

        double pr = priorityOf(populations[state], 1);
        double weight = -pr;  // 取反 trick

        pq.enqueue({state, weight});
    }

    /* 2. 还需要分配 numSeats - k 个席位 */
    int remaining = numSeats - k;

    while (remaining--) {

        // 取出优先级最大的州（权值最小，因为我们使用 -priority）
        DataPoint entry = pq.dequeue();
        string state = entry.name;
        ...
        // 计算该州的新的优先指数
        int s = seats[state];
        double pr = priorityOf(populations[state], s);
