 ...
 * 辅助递归函数
 *
 * remaining : 尚未处理的班次（按值传递，便于删除）
 * chosen    : 当前已选择的班次（按引用）
 * hoursLeft : 剩余可用工时
 * best      : 当前最优结果（按引用）
 * bestValue : 当前最优价值（按引用）
 */
void helper(Set<Shift> remaining,
            Set<Shift>& chosen,
            int hoursLeft,
            Set<Shift>& best,
            int& bestValue) {
    ...
    /* 取出一个 shift 进行分支 */
    Shift s = remaining.first();//从当前还未考虑的班次集合 remaining 中 取出一个 shift（任意）
    remaining.remove(s);//从剩余集合中把这个 shift 删除，因为下面两条递归分支都会“处理”这个 shift，不会再重复处理它

    /**************
     * 分支一：不选 s
     **************/
    helper(remaining, chosen, hoursLeft, best, bestValue);//在“不选当前 shift”的情况下，继续递归处理剩下的班次

    /**************
     * 分支二：尝试选 s
     **************/
    int length = lengthOf(s);   // 使用 lengthOf()

    if (length <= hoursLeft) {
        bool conflict = false;

        /* 检查是否与已选的班次重叠 */
        for (const Shift& sh : chosen) {
            if (overlapsWith(sh, s)) {   // 使用 overlapsWith()
                conflict = true;
                break;
            }
        }
        ...
        if (!conflict) {
            chosen.add(s);
            helper(remaining, chosen, hoursLeft - length, best, bestValue);//假设我们现在 选择了 s，那么剩余可用工时减少了 length，请继续递归处理剩余班次
            chosen.remove(s);   // 回溯
        }
    }
}
...
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    helper(shifts, chosen, maxHours, best, bestValue);
