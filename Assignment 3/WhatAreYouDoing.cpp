 ...
 */
static void helper(const Vector<string>& tokens,int index,string current,Set<string>& result){
    if(index==tokens.size()){
        result.add(current);//把字符串 current 插入到结果集合 result 中。如果已经存在，则不会重复插入（Set 自动去重）。
        return;
    }
    string token=tokens[index];//将当前正在处理的 token（单词、标点、空格等）取出来保存到变量 token 中，便于后续判断和操作
...
Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> tokens = tokenize(sentence);   // 只调用一次 tokenize
    Set<string> result;
    helper(tokens, 0, "", result);//开始递归处理 tokens，从 index = 0 开始，当前构造的字符串是空字符串 ""，所有生成的结果放进 result 里
    return result;
    (void) sentence;
    return {};
}
