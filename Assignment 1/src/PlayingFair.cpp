...
string aSequenceOfOrder(int n) {
    ...
    return aSequenceOfOrder(n - 1)
           + bSequenceOfOrder(n - 1);
}
...
string bSequenceOfOrder(int n) {
    ...
    return bSequenceOfOrder(n - 1)
           + aSequenceOfOrder(n - 1);
}

/*想象一下你身处一片开阔的田野。你从左到右读取一个 A 序列中的字符。
 * 每当你读到 A，你就向前走一步，然后旋转 60°。每当你读到 B，
 * 你就原地转身。重复这个过程会产生复杂精细的结果。代码运行正常后，
 * 运行我们的演示应用程序，看看效果如何！底部的滑块控制序列的顺序。*/
