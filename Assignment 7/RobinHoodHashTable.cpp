...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    this->hashFn= hashFn;
    allocatedSlots=hashFn.numSlots();
    elems=new Slot[allocatedSlots];
    logicalSize=0;
...
bool RobinHoodHashTable::insert(const string& elem) {
    if (logicalSize == allocatedSlots) {
        return false;
    }
    if (contains(elem))
        return false;

    int h = hashFn(elem) % allocatedSlots;
    if (h < 0) h += allocatedSlots;
    string curKey = elem;
    int curDist = 0;

    for (int i = 0; i < allocatedSlots; i++) {
        int idx = (h + i) % allocatedSlots;

        if (elems[idx].distance == EMPTY_SLOT) {
            // place new element
            elems[idx].value = curKey;
            elems[idx].distance = curDist;
            logicalSize++;
            return true;
        }

        if (elems[idx].distance < curDist) {
            std::swap(curKey, elems[idx].value);
            std::swap(curDist, elems[idx].distance);
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for(int i=0; i<allocatedSlots;i++){
        ...
        if(elems[idx].distance < i){
            return false;//当你查找某个 key 时，如果你扫描到的槽位中元素的 distance（离它自己的 home 的距离）
            //比你现在所处位置计算出来的 distance 还小，则可以提前停止，因为你要找的元素不可能在后面。
        }
    }
    return false;
}
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for (int i = 0; i < allocatedSlots; i++) {
        ...
        if (elems[idx].value == elem) {
            ...
            int curr = idx;
            int next = (idx + 1) % allocatedSlots;

            while (elems[next].distance != EMPTY_SLOT &&
                   elems[next].distance > 0) {

                elems[curr] = elems[next];
                elems[curr].distance--;   // 因为向前移动了一格

                curr = next;
                next = (next + 1) % allocatedSlots;
            }
...
void RobinHoodHashTable::printDebugInfo() const {
    std::cout << "Table (" << logicalSize << "/" << allocatedSlots << "):\n";
    for (int i = 0; i < allocatedSlots; ++i) {
        std::cout << "Index " << i << ": ";
        if (elems[i].distance == EMPTY_SLOT) {
            std::cout << "EMPTY\n";
        } else {
            std::cout << elems[i].value << " (Distance: " << elems[i].distance << ")\n";
        }
    }
}
