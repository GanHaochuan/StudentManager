...
/* 内部工具函数：
 * 找到某 key 的插入位置、或判断是否存在
 * 过程：
 *   - 遇到 FILLED 且值相同：返回其下标（found=true）
 *   - 遇到 EMPTY：返回该 EMPTY 的下标（found=false）
 *   - 遇到 TOMBSTONE：记录第一个 T 下标，但继续搜索
 */
int LinearProbingHashTable::findPosition(const string& elem, bool& found) const{
    int h =hashFn(elem);
    found=false;
    int firstTomb=-1;
    for(int i=0;i<allocatedSlots;i++){
        int idx=(h+i)%allocatedSlots;
        if(elems[idx].type==SlotType::FILLED && elems[idx].value==elem){
            found=true;
            return idx;
        }
        if(elems[idx].type==SlotType::TOMBSTONE && firstTomb==-1){
            firstTomb=idx;
        }
        if(elems[idx].type==SlotType::EMPTY){
            found=false;
            return (firstTomb != -1 ? firstTomb : idx);
        }
    }
    return -1;
}
...
bool LinearProbingHashTable::insert(const string& elem) {
    bool found=false;
    int pos = findPosition(elem, found);

    if (found) return false;       // 已存在
    if (pos == -1) return false;   // 表满
...
void LinearProbingHashTable::printDebugInfo() const {
    cout << "Table (" << logicalSize << "/" << allocatedSlots << "):" << endl;

    for (int i = 0; i < allocatedSlots; i++) {
        cout << i << ": ";
        if (elems[i].type == SlotType::EMPTY) cout << "EMPTY";
        else if (elems[i].type == SlotType::TOMBSTONE) cout << "TOMBSTONE";
        else cout << elems[i].value;
        cout << endl;
    }
}
